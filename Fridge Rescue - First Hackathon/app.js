...
let recipes = [];
let favorites = JSON.parse(localStorage.getItem("fridgeRescueFavorites") || "[]");
// ==========================================
...
function normalizeFavorites() {
    favorites =
        favorites.map(favorite => {
            return {
                ...favorite,
                prepTime: favorite.prepTime || 30,
                availableIngredients: favorite.availableIngredients || 0,
                missingIngredients: favorite.missingIngredients || 0,
                ingredients: favorite.ingredients || [],
                measures: favorite.measures || [],
                instructions: favorite.instructions || ""
            };
        });
    localStorage.setItem("fridgeRescueFavorites", JSON.stringify(favorites));
}
// ==========================================
...
function getCacheKey(ingredients) {
    const sortedIngredients = [...ingredients]
        .sort()
        .join("_");
    return (`fridgeRescueMealDB_${CACHE_VERSION}_`
        +
            sortedIngredients);
}
// ==========================================
// GET CACHED RECIPES
// ==========================================
function getCachedRecipes(ingredients) {
...
// ==========================================
function saveRecipesToCache(ingredients, recipeResults) {
    const cacheKey = getCacheKey(ingredients);
    const cache = {
    ...
    console.log("Recipes saved to cache.");
}
// ==========================================
// SEARCH RECIPES
// ==========================================
async function searchRecipes() {
    ...
    // CHECK CACHE
    // ======================================
    const cachedRecipes = getCachedRecipes(selectedIngredients);
    if (cachedRecipes) {
        recipes =
            cachedRecipes;
        sortRecipes("match");
    ...
    try {
        ...
        for (const ingredient of selectedIngredients) {
            const response = await fetch(`${API_URL}/filter.php?i=${encodeURIComponent(ingredient)}`);
            if (!response.ok) {
            ...
            if (data.meals) {
                for (const meal of data.meals) {
                    const currentScore = candidateScores.get(meal.idMeal) || 0;
                    candidateScores.set(meal.idMeal, currentScore + 1);
                }
            }
        }
        // ======================================
        ...
        // ======================================
        const recipePromises = candidateIds.map(id => getRecipeDetails(id));
        const recipeResults = await Promise.all(recipePromises);
        recipes =
            recipeResults.filter(recipe => recipe !== null);
        // ======================================
        ...
        // SAVE TO CACHE
        // ======================================
        saveRecipesToCache(selectedIngredients, recipes);
        statusMessage.textContent =
            "";
        displayRecipes();
    }
...
async function getRecipeDetails(id) {
    ...
    // ======================================
    const prepTime = estimatePrepTime(ingredients.length, meal.strInstructions || "");
    return {
        id: meal.idMeal,
        name: meal.strMeal,
...
// ESTIMATE PREPARATION TIME
// ==========================================
function estimatePrepTime(ingredientCount, instructions) {
    ...
    estimatedTime +=
        Math.min(instructionSteps * 2, 30);
    // Round to nearest 5 minutes
    estimatedTime =
        Math.round(estimatedTime / 5) * 5;
    if (estimatedTime < 15) {
        estimatedTime =
            15;
    }
...
// ==========================================
function userHasIngredient(recipeIngredient) {
    ...
    return selectedIngredients.some(selectedIngredient => {
        ...
        return (normalizedRecipeIngredient ===
            normalizedSelected
            ||
...
function calculateMatchScores() {
    for (const recipe of recipes) {
        ...
        for (const recipeIngredient of recipe.ingredients) {
            if (userHasIngredient(recipeIngredient)) {
                available++;
            }
        }
        const totalIngredients = recipe.ingredients.length;
        const missingIngredients = totalIngredients
            -
                available;
...
function displayRecipes() {
    ...
    for (const recipe of recipes) {
        ...
        card.className =
            "recipe-card";
        const favoriteStatus = isRecipeFavorite(recipe.id);
...
function openRecipe(id) {
    ...
    for (let i = 0; i < recipe.ingredients.length; i++) {
        ...
        const userHas = userHasIngredient(ingredient);
        ingredientsHTML += `
    ...
    const modalFavoriteHeart = document.getElementById("modalFavoriteHeart");
    ...
    modalFavoriteButton.addEventListener("click", () => {
        ...
        const nowFavorite = isRecipeFavorite(recipe.id);
        if (nowFavorite) {
...
// FAVORITES
// ==========================================
function toggleFavorite(id, recipeToAdd) {
...
function sortRecipes(option) {
    ...
    if (option === "match") {
        recipes.sort((a, b) => {
            if (a.missingIngredients !==
                b.missingIngredients) {
                return (a.missingIngredients
                    -
                        b.missingIngredients);
            }
            if (a.prepTime !==
