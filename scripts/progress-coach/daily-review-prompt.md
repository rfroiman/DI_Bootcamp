...
- **Not set up yet:** if `.progress/state.json` doesn't exist, this repo hasn't been bootstrapped. Do not attempt Bootstrap yourself here — that's a separate, manual, one-time step. Make no file changes, make no commit, and end noting that setup hasn't been run yet.

- **Same-day duplicate fire (STRICT):** read `last_run_at` from `state.json`. If its date is already today, a review already ran today. End the session noting that, and make no file changes and no commit — even if new commits have landed since that review. One review per calendar day; the next day's run picks up anything skipped.
...
2. Run `git log <last_reviewed_sha>..HEAD` for new commits, and `git status` for uncommitted local changes. Keep these two categories separate. Commits whose message begins with `progress-coach:` are the coach's own — exclude them from the student's work.
